...
def test_forms_tem_maxlength_30(client: TestClient):
    ...
    # Deep link legado /conta → editar
    r_conta = client.get(f"/p/{part['token']}/conta", follow_redirects=False)
    assert r_conta.status_code == 303
    assert "/meu-perfil/editar" in r_conta.headers["location"]
