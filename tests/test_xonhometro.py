...
def test_admin_registra_saida_e_volta_e_stats(client: TestClient):
    ...
    assert "style.css?v=328" in (ROOT_DIR / "templates" / "base.html").read_text(encoding="utf-8")
...
def test_menu_tem_xonhometro(client: TestClient):
    ...
    chrome = (ROOT_DIR / "static" / "site-chrome.js").read_text(encoding="utf-8")
    assert "thdfm-site-menu-groups-v5" in chrome
    # Saiu do Portal
